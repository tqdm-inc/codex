...
- `mcpServer/oauth/logout` — remove stored OAuth credentials for a configured streamable HTTP MCP server; pass `threadId` to resolve a thread-scoped server and receive `{ removed }` indicating whether credentials existed.
